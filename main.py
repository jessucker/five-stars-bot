...
async def on_message(message):
    ...
    if len(message_log[user_id]) >= REQUIRED_MESSAGES:
        ...
        if role and role not in message.author.roles:
            ...
            await message.channel.send(f"{message.author.mention} just gained the Chatty Patty role!")
...
async def time(ctx, *args):
    ...
    if not args:
        ...
        for abbr, (tz_name, label) in zones.items():
            ...
            output += f"<:fsstar:1376278392558715000> **`{abbr.upper()}`** — {label.lower()}: **{time_str}**\n"
        ...
        try:
            ...
            if input_zone not in zone_abbr:
                await ctx.send("<:fserror:1376184255171793057> i don’t recognize that timezone! try one like `est`, `pst`, `cst`, `mst`, or `gmt` instead.")
                return
            ...
            desc = f"<:fsstar:1376278392558715000> **{parsed_time.strftime('%-I:%M %p')} `{input_zone.upper()}`** is **<t:{unix_ts}:t>** where you live.\n\n"
            ...
            for abbr, (tz_name, label) in zones.items():
                ...
                desc += f"\u2003<:fsstar:1376278392558715000> **`{abbr.upper()}`** — {label.lower()}: **{converted}**\n"
            ...
            await ctx.send("<:fserror:1376184255171793057> i couldn’t understand that! try `!time 2:35pm est` or `!time 5pm pst` instead.")
...
async def horny(ctx, member: discord.Member = None):
    ...
    if bonk_cooldowns.get(ctx.author.id) and (now - bonk_cooldowns[ctx.author.id]).total_seconds() < COOLDOWN_SECONDS:
        await ctx.send("<:fserror:1376184255171793057> you just sent someone to horny jail! wait a few minutes, karen.")
        return
    if booster_role not in ctx.author.roles:
        await ctx.send("<:fserror:1376184255171793057> this command is for hype squad members only. boost the server to abuse your power!")
        return
    if member is None:
        await ctx.send("<:fserror:1376184255171793057> you need to mention someone to bonk.")
        return
    if HORNY_ROLE in [r.name for r in member.roles]:
        await ctx.send("<:fserror:1376184255171793057> babe. they’re already locked up. take a seat.")
        return
...
async def shitty(ctx, member: discord.Member):
    ...
    if not role:
        await ctx.send("<:fserror:1376184255171793057> the shithead prison role doesn’t exist. check with an admin!")
        return
...
async def shitty_error(ctx, error):
    if isinstance(error, commands.CommandOnCooldown):
        await ctx.send("<:fserror:1376184255171793057> you just sent someone to shithead prison! wait a few minutes, karen.")
